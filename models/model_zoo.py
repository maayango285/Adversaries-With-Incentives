...
class PreActResNet(nn.Module):
  ...
  def __init__(self,
    ...
    self.mean = torch.tensor(STD_MEAN_DICT[dataset]['mean']).view(num_input_channels, 1, 1)
    self.std = torch.tensor(STD_MEAN_DICT[dataset]['std']).view(num_input_channels, 1, 1)
    self.mean_cuda = self.mean.to(torch.device("cuda"))#&&&?None
    self.std_cuda = self.std.to(torch.device("cuda"))#&&&?None
    self.padding = padding
  ...
  def forward(self, x):
    ...
    out = self.relu(self.batchnorm(out))
    out = self.avg_pool(out) #out = F.avg_pool2d(out, 4) #&&&?
